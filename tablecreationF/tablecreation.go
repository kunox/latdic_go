...
import (
	...
	"strings"
	"github.com/kunox/latdic_go/fulldict"
	Ending"github.com/kunox/latdic_go/endinglist"
	"strconv"
)
...
func GetVtable(exdc S.ExdicContent) [][]string {
	...
	tname := sa2[0]
	 ...
	 switch {
	 case spnote == "DEP" : return exceptionalDEP(ret)
	 case spnote =="SEMIDEP" : return exceptionalSEM(ret)
	 case spnote == "PERFDEF" : return exceptionalPERFDEF(ret)
...
func GetPPLTable(exdc S.ExdicContent) [][]string {
	...
	if num =="0" || num =="5"{
		...
		case '1', '2', '3' : tname = "VPAR" + string(tname[1]) + "0"
		case '7' : tname = "VPAR62"
		default: tname = "VPAR" + tname[1:]
		}
	} else if num == "2" || num == "4" {
...
func GetPRONTable(dc S.DicContent) [][]string {
	...
	enclitic := ""
	switch tname {
	case "PRON21": { tname = "PRON11"
		enclitic = getEnclitic(dc)}
	case "PRON22": {tname ="PRON21"
		enclitic = getEnclitic(dc)}
